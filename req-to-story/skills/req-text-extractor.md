...
Used by `req-ingestor.agent.md` to normalise all input files into plain text before analysis.
...
ACTION REQUIRED — choose one of the following:
  Option A: Export as PDF and re-run:
            /gen-user-stories --req <path-to-pdf>
  Option B: Export as Markdown/plain text and re-run:
            /gen-user-stories --req <path-to-md>
...
All extraction results are passed to `req-ingestor.agent.md` as an array.
