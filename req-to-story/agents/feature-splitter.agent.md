...
**Role**: Step 3 of the `/gen-user-stories` pipeline. Reads the analyzed requirement document
...
⚠️  Feature count (N) exceeds limit (M).
    ...
    Run /gen-user-stories again with --limit <higher-N> to include them.
```
...
- `acceptanceCriteriaDraft` is a raw extraction — `user-story-writer` will refine it into
  proper AC statements following the quality rules
