...
This file is loaded by CLAUDE.md when `kit.config.json` has `"id": "kit-u1"`.
...
## Kit-U1 Upgrade to Kit-U2

If the user already has a Playwright project and wants to integrate:
- Switch `kit.config.json` `"id"` to `"kit-u2"` and `"extends"` to `"kit-u1"`
- The framework-analyzer-agent will scan existing structure before generating
- Do NOT overwrite existing `playwright.config.ts` — patch it instead
