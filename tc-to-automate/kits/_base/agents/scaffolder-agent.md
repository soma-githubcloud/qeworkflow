...
Load templates from `kitConfig.templates`:
...
**`playwright.config.ts`** from `playwright.config.ts.tmpl`:
...
**`allure.config.ts`** from `allure.config.ts.tmpl`:
...
Used by Kit-U2, Kit-U4. Before scaffolding, read `tc-to-automate/kits/<kitId>/agents/framework-analyzer-agent.md`
and execute it first to discover the existing structure. The analyzer agent returns a
`frameworkMap` that this agent uses to decide:
