...
`quality-master-orchestrator` as Phase 1, or directly via `/gen-manual-tests`.
...
All agent definitions live in `story-to-testcase/agents/specialists/`.

## Input Parameters

```
userStoryText   : string  — full user story content (NOT a path — caller reads file first)
projectName     : string  — from kit.config.json or --project
...
1. Read the agent definition from `story-to-testcase/agents/specialists/<name>.agent.md`
