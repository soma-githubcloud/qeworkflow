...
|---|---|---|
| `intake-agent` | `kits/_base/agents/intake-agent.md` | Normalize inputs → `intake.summary.json` |
| `locator-agent` | `kits/_base/agents/locator-agent.md` | Produce `selectors.json` via 3-tier pipeline |
| `pom-agent` | `kits/_base/agents/pom-agent.md` | Generate `*Page.ts` POM files from `selectors.json` |
| `testgen-agent` | `kits/_base/agents/testgen-agent.md` | Generate non-BDD `.spec.ts` files |
| `bdd-agent` | `kits/_base/agents/bdd-agent.md` | Generate `.feature` + `.steps.ts` together |
| `datagen-agent` | `kits/_base/agents/datagen-agent.md` | Generate faker-js factories + data fixtures |
| `ci-agent` | `kits/_base/agents/ci-agent.md` | Generate GitHub Actions / AzDO / GitLab YAML |
| `scaffolder-agent` | `kits/kit-u1/agents/framework-setup-agent.md` | Create project structure + config files |
| `reporting-agent` | `kits/_base/agents/reporting-agent.md` | Wire Allure + HTML reporters |
...
| Skill | File | Purpose |
|---|---|---|
| `selector-scorer` | `kits/_base/skills/selector-scorer.md` | Numeric 0.99→0.50 scoring; lint threshold at 0.70 |
| `locator-strategies` | `kits/_base/skills/locator-strategies.md` | Playwright selector priority rules; what to prefer and avoid |
| `gherkin-transformer` | `kits/_base/skills/gherkin-transformer.md` | Maps TC steps → Given/When/Then; enforces domain language guard |
| `data-factory` | `kits/_base/skills/data-factory.md` | faker-js field patterns; positive/boundary/negative data sets |
| `ci-scaffolder` | `kits/_base/skills/ci-scaffolder.md` | GitHub Actions YAML patterns; secrets injection; matrix strategy |
| `test-case-parser` | `kits/_base/skills/test-case-parser.md` | Parses 4 input formats into structured TC objects |
| `template-renderer` | `kits/_base/skills/template-renderer.md` | Handlebars `{{var}}` / `{{#if}}` / `{{#each}}` rendering rules |
...
`configs/integrations.yml` — never hardcoded.
...
**New kit checklist**:
1. Create `kits/<kit-id>/kit.config.json` with `"extends"` pointing to parent
2. Add `kits/<kit-id>/KIT.md` with tech-specific naming and code shape rules
3. Add `kits/<kit-id>/templates/` for any new file types
