...
## Selenium Locator API Mapping (Kit-U3, U4, H2)

```java
Priority 1-2:  By.cssSelector("[data-testid='submit-btn']")
Priority 3:    By.cssSelector("[role='button'][aria-label='Sign in']")
Priority 4:    By.cssSelector("[aria-label='Close dialog']")
Priority 5:    By.name("username")
Priority 6:    By.cssSelector("[placeholder='Enter email']")
Priority 7:    By.id("loginBtn")
Priority 8:    By.cssSelector(".login-form__submit")
Priority 9:    By.xpath("//button[normalize-space()='Submit']")  // text match
Priority 10:   By.xpath("//form/div[2]/button")  // NEVER do this
```

---

## CDP Locator Strategy (Kit-U3 Tier-1)

When using Chrome DevTools Protocol for live locator extraction (Kit-U3/U4):
1. Open browser with CDP enabled: `ChromeOptions` with `--remote-debugging-port=9222`
2. Use Selenium IDE recording session to capture user interactions and auto-generate locators
3. Export locators in the `By.*` format
4. Apply the priority table above to select the best locator from CDP-captured candidates

---

## Locator Naming Conventions

| Element Type | TypeScript Name | Java Name |
|---|---|---|
| Text input | `emailInput`, `passwordInput` | `emailInput`, `passwordInput` |
| Submit button | `submitButton`, `loginButton` | `submitButton`, `loginButton` |
| Link | `forgotPasswordLink` | `forgotPasswordLink` |
| Dropdown | `countrySelect`, `roleDropdown` | `countrySelect`, `roleDropdown` |
| Checkbox | `rememberMeCheckbox` | `rememberMeCheckbox` |
| Radio button | `adminRoleRadio` | `adminRoleRadio` |
| Error/alert | `errorMessage`, `validationAlert` | `errorMessage`, `validationAlert` |
| Success message | `successBanner`, `confirmationText` | `successBanner`, `confirmationText` |
| Table | `usersTable`, `productGrid` | `usersTable`, `productGrid` |
| Modal | `confirmModal`, `deleteDialog` | `confirmModal`, `deleteDialog` |
| Page heading | `pageTitle`, `sectionHeading` | `pageTitle`, `sectionHeading` |
